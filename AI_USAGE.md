...
- ran the live smoke test and the full live evaluation against the real OpenAI API. Before sourcing `.env`, key presence was checked with a boolean grep (`grep -q '^OPENAI_API_KEY=.\+' .env`) that never printed the value; the key was then exported only into one command subshell (`set -a && source .env && set +a && uv run ...`) and never echoed, logged, or written to any committed file. Two ambiguity questions correctly produced live clarifications; one explicit all-time ranking question completed the full live path (generated → validated → authorized → executed → rendered), with the top result matching development anchor A1 exactly. The full 13-question set scored 9/13 on the first live pass; investigating the failures found one genuine product defect (a correct, business-rule-compliant `CASE`/`CAST` SQL expression for average ticket price was wrongly rejected as an "unrecognized function" — `exp.Case`/`exp.If`/`exp.Cast` are `Func`-derived in the pinned SQLGlot version, the same bug class already fixed once for `And`/`Or`/`Exists`), fixed narrowly with regression tests, and reran to 10/13; the remaining 3 were classified as expected model behavior (over-cautious clarification, the model refusing a prompt-injection attempt outright, and a case only meaningful in reference mode), not defects, and were left unmodified. `evaluation/results_live.md` records the full breakdown.
