...
| Suite | Decision/prompt-focused tests: 100 under `tests/unit/decision/` (includes the revenue-ranking ambiguity-policy regression suite); 69 metadata; 369 SQL-policy; 403 under `tests/unit/db` (includes U3 authorizer/execution coverage); 14 under `tests/unit/service` (mocked end-to-end QueryService); 11 under `tests/unit/cli`; 18 comparison tests under `tests/unit/evaluation`; 989 in the full offline suite; Ruff lint and format; mypy strict; `uv lock --check`; `git diff --check`; credential-pattern scan clean on changed paths. |
