...
```bash
git clone https://github.com/robin-raq/bse-nlq-agent.git
cd bse-nlq-agent
cp .env.example .env   # set OPENAI_API_KEY
make dev               # sync, build DB if needed, interactive examples
```

`make dev` launches `bse-nlq ask` with no question: pick a numbered example
or type your own. One-shot usage still works:
...
# or: make ask Q='Show me the top 5 event categories by total revenue.'
```
...
No API key yet? Run the offline reference demo (no network, no credits):
...
A safe failure, transparently shown, not a crash. Two separate facts are easy
to conflate here: "Barclays Center" is also absent from this synthetic
dataset (seeded venues are invented names like Kings Harbor Arena — see
[Dataset](#dataset)), so even allowlist-legal SQL filtered on that venue
would have returned an empty result rather than inventing rows. Separately,
under **prompt policy version 1** this question shape led the model to use a
`MAX()` subquery (a real SQLite function outside the deliberately narrow
3-function allowlist — see [Safety](#safety)). The static policy and the
SQLite authorizer both independently reject it before it ever reaches the
database. Prompt policy version 2 now names that allowlist and the integer
weighted-average formula; this exact PRD phrasing was **not re-run live
under version 2**. See [Limitations](#limitations) and
`evaluation/results_live.md` for the honest detail, including a second live
attempt that hit the same boundary a different way (`NULLIF`). Unlike the
Nets example above, the model did not ask how to resolve the unknown venue;
it emitted filter SQL and the safety layers stopped execution.
...
executed SQL matches what the model produced apart from outer-whitespace
trimming in the validator.
...
- Narrow function allowlist, demonstrated historically by PRD question 3
  above under **prompt policy version 1**: a ranked/superlative "average
  ticket price" question led the model to `NULLIF` or a `MAX()` subquery,
  both real SQLite functions outside the deliberately narrow 3-function
  allowlist (`SUM`, `COUNT`, `COALESCE`), so the query was safely rejected.
  Prompt policy version 2 names that allowlist and supplies the integer
  weighted-average formula; the compact evaluation and paired comparison
  show OpenAI accepting SQL policy on average-ticket cases. The exact PRD
  Barclays phrasing was not re-run live under version 2. The simpler
  (non-ranked) "average ticket price" phrasing reliably succeeds.
